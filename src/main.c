...
void clean_up(int totalCustomers, void **threadIDS, int errorCode) {
...
double timespec_to_minutes(struct timespec ts) {
    double total_minutes = (double) ts.tv_sec / SECONDS_PER_MINUTE;

    total_minutes += (double) ts.tv_nsec / NANOSECONDS_PER_SECOND / SECONDS_PER_MINUTE;
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < totalCustomers; ++i) {
        ...
        ((thread_args_t *)threadIDS[i])->oid = i;
        ((thread_args_t *)threadIDS[i])->seed = seed;
        if (pthread_create(&threads[i], NULL, order, threadIDS[i]) != 0) {
            fprintf(stderr, "Failed to create thread (%d).\n", i);
            clean_up(totalCustomers, threadIDS, THREAD_CREATE_ERROR);
        }
    }
    ...
    for (int i = 0; i < totalCustomers; ++i) {
        if (pthread_join(threads[i], NULL) != 0) {
            ...
            clean_up(totalCustomers, threadIDS, THREAD_JOIN_ERROR);
        }
    }
    ...
    printf("Max service time: %.2f minutes.\n", timespec_to_minutes(max_serv_time));
    ...
    clean_up(totalCustomers, threadIDS, SUCCESS);
}
